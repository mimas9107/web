...
?>
        ...
        <article id="article"
            ...
            if (isset($error) && isset($success))
                echo loadHTML($target, array('config' => $config, 'error' => $error, 'success' => $success));
            else
                echo loadHTML($target, array('config' => $config));
            ?>
        </article>
<?php 
    }
...
<?php
    if($target == 'post-article.php' || $target == 'post-img-article.php') {
?>
    <style type="text/css">
        #article {
            column-count: 1;
